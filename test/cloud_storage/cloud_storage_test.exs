...
defmodule CloudStorageTest do
  ...
  test "create_bucket/1 (default class and region)" do
    expected =
      build_expected(:put, "bucket.#{@endpoint}", [{"x-goog-project-id", @project_id}], "")
  ...
  test "create_bucket/2 (default class and custom region)" do
    ...
    expected =
      build_expected(:put, "bucket.#{@endpoint}", [{"x-goog-project-id", @project_id}], body)
  ...
  test "set_object_acl/4 (with query)" do
    ...
    assert expected ==
             API.set_object_acl(bucket, object, acl_config, [{"key1", "abc"}, {"key2", "def"}])
  end
